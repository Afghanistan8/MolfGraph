...
MAX_EVIDENCE_BYTES = 262144          # 256 KiB per digest-verified evidence item
MAX_TOTAL_EVIDENCE_BYTES = 1048576   # 1 MiB across a single adjudication
...
# ---------------------------------------------------------------------------
# Deterministic helpers (pure functions -- unit-testable offline)
# ---------------------------------------------------------------------------
...
def _untrusted_block(label: str, body: str) -> str:
    ...
    return (
        "=== BEGIN UNTRUSTED DATA [" + safe_label + "] ===\n"
        + safe_body
...
def _source_set_sha256(digests: list) -> str:
    ...
    normalised = sorted(
        [{"sha256": d.get("sha256", ""), "uri": d.get("uri", "")} for d in digests],
        key=lambda d: (d["uri"], d["sha256"]),
    )
...
def _shape_result(
    ...
    if status == "VERIFIED" and (not citation or not clean_sources):
        status = "INSUFFICIENT_EVIDENCE"
        addition = (
            "Downgraded from VERIFIED: a citation and at least one verified "
            "source are required."
        )
    ...
    return {
        ...
        "status": status,
        "citation": citation,
        "citation_key": _norm_citation(citation),
        "exact_text_or_summary": body,
        "applicability_score": score,
        "applicability_bucket": _bucket(score),
        "confidence": confidence,
        "sources": clean_sources,
        "source_set_sha256": _clean(source_set, 64),
        "notes": notes,
        "disclaimer": DISCLAIMER,
        "study_id": _as_int(study_id, 0),
        "study_version": _as_int(study_version, 0),
    }


def _needs_alert(result: dict) -> bool:
    return (
        result.get("status") in ("CONFLICT", "UNAVAILABLE", "INSUFFICIENT_EVIDENCE")
...
PROMPT_ROLE = (
    "You are a legal research assistant supporting qualified lawyers.\n"
    "HARD RULES:\n"
...
PRINCIPLE_ANALYSIS = (
    "The outputs must agree on the value of `status`, on the citation once "
    "normalised for whitespace, letter case and punctuation, on "
    "`applicability_bucket`, and on `confidence`. Wording of "
    "`exact_text_or_summary` and `notes` may differ. Reject any output whose "
    "citation or statutory text does not appear in the UNTRUSTED DATA blocks."
)

PROMPT_RELATION = (
    "You are comparing two immutable legal screening study versions to classify "
    "their relationship. Choose exactly one relation_type:\n"
...
# ---------------------------------------------------------------------------
# VecDB record payload -- retrieval context only, never a graph edge
# ---------------------------------------------------------------------------
...
def _embedder():
    """The pinned sentence-embedding model used for VecDB context retrieval."""
    try:
        return genlayermodelwrappers.SentenceTransformer("all-MiniLM-L6-v2")
    except TypeError:
        return genlayermodelwrappers.SentenceTransformer()
...
class Contract(gl.Contract):
    ...
    # studies and immutable versions
    studies: TreeMap[u256, str]
    study_ids: DynArray[u256]
    study_creator: TreeMap[u256, Address]
    next_study_id: u256

    versions: TreeMap[str, str]          # "study:version" -> version JSON
    ...
    # relation claims and accepted edges
    claims: TreeMap[u256, str]
    claim_ids: DynArray[u256]
    next_claim_id: u256

    edges: TreeMap[u256, str]            # accepted edges only
    ...
    # privacy-preserving matter registry
    ...
    # semantic context index -- never authoritative
    vecdb: VecDB[np.float32, typing.Literal[384], RecordValue]
    ...
    def _gate_urls(
        ...
        country: str,
        raw: typing.Any,
        extra_countries: typing.Any = None,
    ) -> tuple:
    ...
    def _receipt(
        ...
        self.receipts.append(_canon_json({
            ...
            "tx_context": _clean(tx_context, MAX_TITLE),
    ...
    def _alert(self, kind: str, severity: str, message: str, refs: dict) -> None:
        self.alerts.append(_canon_json({
            "alert_id": len(self.alerts) + 1,
            "kind": _clean(kind, 64),
            "severity": severity if severity in ("LOW", "MEDIUM", "HIGH") else "MEDIUM",
            "message": _clean(message, MAX_TITLE * 2),
            "refs": refs,
            "actor": self._sender(),
            "created_at": self._now(),
            "seq": self._seq(),
    ...
    def _store_analysis(self, result: dict, country: str, inputs_sha256: str) -> int:
        ...
        self._receipt(
            "ANALYSIS",
            [record.get("study_id", 0)],
            [record.get("study_version", 0)],
        ...
        if _needs_alert(record):
            self._alert(
                "ANALYSIS_" + str(status),
                "HIGH" if status == "CONFLICT" else "MEDIUM",
                record.get("kind", "") + " returned " + str(status)
                + " with " + str(record.get("confidence")) + " confidence.",
                {"analysis_id": analysis_id, "country": country},
            )
        return analysis_id
    ...
    def _neighbor_context(self, text: str, k: int) -> list:
        """VecDB neighbours, labelled context-only. Never used to decide."""
        query = _clean(text, MAX_TEXT)
        if not query:
            return []
        try:
            embedding = _embedder()(query)
            found = []
            for item in self.vecdb.knn(embedding, max(1, min(k, MAX_KNN))):
                value = item.value
                found.append({
                    "record_id": value.record_id,
                    "study_id": int(value.study_id),
                    "version": int(value.version),
                    "record_type": value.record_type,
                    "text": _clean(value.text, 600),
    ...
    def _neighbor_block(self, neighbors: list) -> str:
        if not neighbors:
            return ""
        lines = [
            "RELATED RECORDS (semantic context only -- NOT evidence, NOT an edge,",
            "and NOT a source you may cite):",
        ]
        for n in neighbors:
            lines.append(
                "- [" + n["record_type"] + " study " + str(n["study_id"])
                + " v" + str(n["version"]) + "] " + _norm_ws(n["text"])[:400]
            )
        return "\n".join(lines)
    ...
    def _run_analysis(
        ...
        inputs_sha = _sha256_text(_canon_json({
            "kind": kind,
            "country": country,
            "task": task,
            "urls": urls,
        ...
        if failure or not urls:
            result = _shape_result(
                ...
                {
                    "status": "UNAVAILABLE",
                    "notes": "No usable trusted source. Reason: " + (failure or "NO_SOURCES")
                    + ". MolfGraph refuses to answer without grounded public sources.",
                    "confidence": "LOW",
                },
                [],
                _source_set_sha256([]),
        ...
        def leader_fn() -> str:
            ...
            if not used:
                return _canon_json(_shape_result(
                    kind,
                    {
                        "status": "UNAVAILABLE",
                        "notes": "Every trusted source was unreachable at read time.",
                        "confidence": "LOW",
                    },
                    [],
                    _source_set_sha256([]),
            ...
            parts.extend([
                "",
                "Answer the TASK using only the UNTRUSTED DATA above. Return STRICT JSON.",
            ])
            ...
            shaped = _shape_result(
                ...
                _extract_json(raw),
                used,
                _source_set_sha256(digests),
                study_id,
                study_version,
            )
            if strict:
                # Strict extraction agrees byte-for-byte on a canonical key that
                # carries the normalised law text itself.
        ...
        try:
            if strict:
                ...
                consensus_raw = gl.eq_principle.prompt_comparative(
                    leader_fn, principle=PRINCIPLE_ANALYSIS
                )
        except Exception:
            consensus_raw = ""
        ...
        if not parsed:
            parsed = {
                "status": "INSUFFICIENT_EVIDENCE",
                "notes": "Validators did not reach consensus on this analysis.",
                "confidence": "LOW",
            }
            sources_out = []
    ...
    def _build_records(self, study_id: int, version: int, records_json: str) -> list:
        ...
        for idx, item in enumerate(raw):
            ...
            built.append({
                "record_id": str(study_id) + ":" + str(version) + ":" + str(idx),
    ...
    def _write_version(
        ...
        payload = {
            ...
            "crime_or_charge": crime_or_charge,
            "question": question,
            "method": method,
    ...
    def register_study_version(
        ...
        payload = self._write_version(
            study_id, 1, 0, title, country, subject_ref, crime_or_charge,
            question, method, conclusion, records, "",
        )

        self.studies[u256(study_id)] = _canon_json({
            "study_id": study_id,
            "title": title,
            "country": country,
            "crime_or_charge": crime_or_charge,
        ...
        self._receipt(
            "STUDY_VERSION", [study_id], [1], payload["content_sha256"], "",
    ...
    def correct_study(
        ...
        payload = self._write_version(
            sid, new_version, previous, title, country, subject_ref, crime_or_charge,
            question, method, conclusion, records, correction_note,
        )
        ...
        self._receipt(
            "CORRECTION", [sid], [previous, new_version],
            payload["content_sha256"], "", "correct_study",
        )
        return _canon_json(payload)

    @gl.public.write
    def index_study_records(self, study_id: u256, version: u256) -> str:
        """Index a pinned version's records into VecDB for context retrieval.

        Historical vectors are never deleted: a correction adds new vectors so
        provenance across versions stays intact.
        """
        sid = int(study_id)
        ver = int(version)
        ...
        for record in snapshot.get("records", []):
            text = _clean(record.get("text", ""), MAX_TEXT)
            if not text:
                continue
            self.vecdb.insert(embed(text), RecordValue(
            ...
            indexed.append(record.get("record_id", ""))

        self._receipt(
            "STUDY_VERSION", [sid], [ver],
            snapshot.get("content_sha256", ""), "", "index_study_records",
        )
        return _canon_json({
            "study_id": sid,
            "version": ver,
            "indexed_record_ids": indexed,
            "note": "VecDB entries are semantic context only. They never create edges.",
            "disclaimer": DISCLAIMER,
        })

    # -- source-grounded legal screening tools ------------------------------
    ...
    def verify_statute(self, crime_or_charge: str, country: str, trusted_urls: str = "") -> str:
        ...
        task = (
            "Identify the official statute or code section that defines the offence "
            "or charge described as: " + charge + " in jurisdiction " + _norm_country(country)
            + ". Report the official citation and the defining provision. Do not assess "
            "any individual's conduct."
        )
        return self._run_analysis(
            "verify_statute", country, task, trusted_urls, context_seed=charge
        )
    ...
    def screen_application(
        ...
        task = (
            "A lawyer describes how a law was applied. Compare that description "
            "against the statutory elements and public case material in the sources.\n"
            "CHARGE OR LAW: " + charge + "\n"
            "DESCRIBED APPLICATION: " + facts + "\n"
            "Report whether the described application is consistent with the elements "
            "as published. State every element that the sources do not cover. "
            "You must NOT state or imply whether any person is guilty, innocent, "
            "liable or not liable, and you must NOT predict a case outcome."
        )
        return self._run_analysis(
            "screen_application", country, task, trusted_urls, context_seed=charge + " " + facts
        )

    @gl.public.write
    def extract_law_text(
        self,
        provision_or_citation: str,
        country: str,
        trusted_urls: str = "",
    ) -> str:
        ...
        task = (
            "Extract the exact current text of provision " + provision
            + " in jurisdiction " + _norm_country(country)
            + ". Copy the operative text verbatim from the sources into "
            "exact_text_or_summary. Do not paraphrase, summarise, compare or comment. "
            "If the exact text is not present in the sources, return "
            "INSUFFICIENT_EVIDENCE with an empty exact_text_or_summary."
        )
        return self._run_analysis(
            "extract_law_text", country, task, trusted_urls, strict=True
        )
    ...
    def compare_jurisdictions(
        ...
        task = (
            "Compare how jurisdiction " + a + " and jurisdiction " + b
            + " treat: " + topic + ". Report the official citation in each jurisdiction "
            "and the material differences that a lawyer must account for. Use the "
            "citation field for the primary provision in " + a + ". If either "
            "jurisdiction is not covered by the sources, return INSUFFICIENT_EVIDENCE."
        )
        # Both jurisdictions' registries are in scope, so a caller-supplied URL
        # from either side passes the origin gate while everything else fails closed.
        return self._run_analysis(
            "compare_jurisdictions", a, task, trusted_urls,
            context_seed=topic, extra_countries=[b],
        )
    ...
    def check_statute_of_limitations(
        ...
        if not charge:
            raise Exception("MolfGraph: crime_or_charge required")
        task = (
            "Report the statutory limitation period that the sources publish for: "
            + charge + " in jurisdiction " + _norm_country(country)
            + ". Give the provision that sets the period, the period itself, and any "
            "published tolling or extension rules. Do not compute a deadline for any "
            "specific matter."
        )
        return self._run_analysis(
            "check_statute_of_limitations", country, task, trusted_urls, context_seed=charge
        )

    @gl.public.write
    def check_conflicts(
        self,
        citation_or_provision: str,
        country: str,
        trusted_urls: str = "",
    ) -> str:
        ...
        task = (
            "Determine the current standing of " + provision + " in jurisdiction "
            + _norm_country(country) + ". State whether the sources show it as IN FORCE, "
            "AMENDED, REPEALED, SUPERSEDED, or in CONFLICT with another instrument, and "
            "cite the instrument that changed it. If the sources disagree with each "
            "other, return status CONFLICT and describe the disagreement."
        )
        return self._run_analysis(
            "check_conflicts", country, task, trusted_urls, context_seed=provision
        )
    ...
    def map_facts_to_provisions(
        ...
        task = (
            "Map the described fact pattern to the published provisions that a lawyer "
            "should review in jurisdiction " + _norm_country(country) + ".\n"
            "FACT PATTERN: " + facts + "\n"
            "List candidate provisions with official citations, and state for each which "
            "elements the description does and does not address. Do not conclude that "
            "any offence was or was not committed."
        )
        return self._run_analysis(
            "map_facts_to_provisions", country, task, trusted_urls, context_seed=facts
        )
    ...
    def generate_verification_report(self, study_id: u256, version: u256, country: str) -> str:
        ...
        for record in snapshot.get("records", []):
            sections.append(
                "[" + record.get("record_type", "") + "] " + _norm_ws(record.get("text", ""))[:800]
            )
        task = (
            "Verify the following pinned legal screening study version against the "
            "official sources. Report whether its stated conclusion is supported by the "
            "published law, and list every claim the sources do not support.\n"
        ...
        return self._run_analysis(
            "generate_verification_report", country, task, "",
            study_id=sid, study_version=ver,
            context_seed=snapshot.get("question", "") + " " + snapshot.get("conclusion", ""),
        )
    ...
    def register_evidence(
        ...
        return self._write_evidence(
            stable_id, uri, digest, ver, origin, _clean(issued_at, 64),
            _clean(record_type, 32).upper(), "REGISTER",
        )
    ...
    def _write_evidence(
        ...
        payload = {
            ...
            "issued_at": issued_at,
            "record_type": record_type if record_type in RECORD_TYPES else "",
            "mode": mode,
    ...
    def repair_evidence(
        ...
        return self._write_evidence(
            stable_id, uri, digest, ver, current.get("publisher_origin", ""),
            current.get("issued_at", ""), current.get("record_type", ""), "REPAIR",
        )
    ...
    def propose_relation(
        ...
        payload = {
            ...
            "status": "PENDING",
            "from_content_sha256": source.get("content_sha256", ""),
            "to_content_sha256": target.get("content_sha256", ""),
        ...
        self._receipt(
            "EDGE", [from_sid, to_sid], [from_ver, to_ver],
            _sha256_text(_canon_json(payload)), "", "propose_relation",
        )
        return _canon_json(payload)
    ...
    def adjudicate_relation(self, claim_id: u256) -> str:
        ...
        study_block = (
            "STUDY A -- id " + str(from_sid) + " version " + str(from_ver) + "\n"
        ...
        def evaluate_once() -> str:
            ...
            for item in evidence_items:
                ...
                if failure:
                    ...
                    out.update({
                        ...
                        "relation_type": "INSUFFICIENT",
                        "confidence": "LOW",
                        "evidence_pass": False,
                        "source_set_sha256": "",
                        "failure_code": failure,
                        "failed_evidence_id": int(item["evidence_id"]),
                        "observed_sha256": "",
                    })
            ...
            prompt = "\n".join([
                ...
                PROMPT_RELATION,
                "",
                "The claimant proposes: " + claim.get("claimed_relation", ""),
                "Judge the claim independently. Agreeing with the claimant is not required.",
                "",
                study_block,
                "",
                "VERIFIED EVIDENCE (digest-matched public sources):",
                "\n".join(blocks),
            ])
            ...
            out = dict(endpoints)
            out.update({
                "status": "REJECTED_AS_INSUFFICIENT" if relation == "INSUFFICIENT" else "ACCEPTED",
                ...
                "observed_sha256": "",
                "rationale": _clean(parsed.get("rationale", ""), MAX_TEXT),
            })
            return _canon_json(out)

        def validator_fn(leader_result: typing.Any) -> bool:
            mine = _extract_json(evaluate_once())
            theirs = _extract_json(
                leader_result if isinstance(leader_result, (str, dict))
                else _canon_json(leader_result)
            )
            ...
            fields = [
                ...
                "confidence", "source_set_sha256", "evidence_pass",
            ]
        ...
        if not decision:
            ...
            decision.update({
                ...
                "evidence_pass": False,
                "source_set_sha256": "",
                "failure_code": "NO_CONSENSUS",
                "failed_evidence_id": 0,
                "observed_sha256": "",
                "rationale": "Validators did not agree on the decision-critical fields.",
            })
    ...
    def _settle_claim(self, cid: u256, claim: dict, decision: dict) -> str:
        ...
        claim["decision"] = {
            ...
            "source_set_sha256": source_set,
            "failure_code": _clean(decision.get("failure_code", ""), 32),
            "failed_evidence_id": _as_int(decision.get("failed_evidence_id", 0), 0),
            "observed_sha256": _clean(decision.get("observed_sha256", ""), 64),
            "rationale": _clean(decision.get("rationale", ""), MAX_TEXT),
        }
        ...
        if status == "REPAIR_REQUIRED":
            ...
            self._alert(
                "EVIDENCE_" + claim["decision"]["failure_code"],
                "HIGH",
                "Claim " + str(int(cid)) + " failed evidence verification ("
                + claim["decision"]["failure_code"] + "). No edge was created.",
                {
                    "claim_id": int(cid),
                    "evidence_id": claim["decision"]["failed_evidence_id"],
                    "observed_sha256": claim["decision"]["observed_sha256"],
                },
            )
            self._receipt(
                "ALERT", [from_sid, to_sid], [from_ver, to_ver],
                _sha256_text(_canon_json(claim["decision"])), source_set,
                "adjudicate_relation repair_required",
            )
        ...
        if status != "ACCEPTED" or relation == "INSUFFICIENT" or not evidence_pass:
            ...
            self._alert(
                ...
                "MEDIUM",
                "Claim " + str(int(cid)) + " did not reach a substantive relation. "
                "No accepted edge was created.",
                {"claim_id": int(cid)},
            )
            self._receipt(
                "EDGE", [from_sid, to_sid], [from_ver, to_ver],
                _sha256_text(_canon_json(claim["decision"])), source_set,
                "adjudicate_relation insufficient",
            )
            return _canon_json({"claim": claim, "edge_id": 0, "disclaimer": DISCLAIMER})

        dedupe_key = (
            str(from_sid) + ":" + str(from_ver) + "->" + str(to_sid) + ":" + str(to_ver)
            + ":" + relation
        )
        ...
        self._receipt(
            "EDGE", [from_sid, to_sid], [from_ver, to_ver],
            _sha256_text(body), source_set, "adjudicate_relation accepted",
        )
        return _canon_json({"claim": claim, "edge": edge, "edge_id": edge_id,
                            "disclaimer": DISCLAIMER})

    # -- privacy-preserving case registry -----------------------------------
    ...
    def search_studies(self, query: str, country: str) -> str:
        ...
        for sid in self.study_ids:
            ...
            if want_country and study.get("country") != want_country:
                continue
            haystack = _norm_citation(
                str(study.get("title", "")) + " " + str(study.get("crime_or_charge", ""))
            )
    ...
    def get_graph(self, limit: u256) -> str:
        ...
        for eid in self.edge_ids:
            ...
            edges.append(edge)
            for sid, ver in (
                (edge["from_study_id"], edge["from_version"]),
                (edge["to_study_id"], edge["to_version"]),
            ):
        ...
        for key in pinned:
            ...
            nodes.append({
                ...
                "crime_or_charge": snapshot.get("crime_or_charge", ""),
                "status": snapshot.get("status", "ACTIVE"),
                "content_sha256": snapshot.get("content_sha256", ""),
                "record_types": sorted({
                    r.get("record_type", "") for r in snapshot.get("records", [])
                }),
            })
    ...
    def get_stats(self) -> str:
        return _canon_json({
            "total_studies": int(self.total_studies),
            "total_versions": int(self.total_versions),
            "total_analyses": int(self.total_analyses),
            "total_edges": int(self.total_edges),
            "total_claims": len(self.claim_ids),
            "total_evidence": len(self.evidence_ids),
            "total_receipts": len(self.receipts),
            "total_cases": len(self.case_ids),
            "stat_verified": int(self.stat_verified),
            "stat_unavailable": int(self.stat_unavailable),
            "stat_insufficient": int(self.stat_insufficient),
            "stat_conflicts": int(self.stat_conflicts),
            "stat_low_confidence": int(self.stat_low_confidence),
            "stat_alerts": int(self.stat_alerts),
            "stat_hash_mismatch": int(self.stat_hash_mismatch),
    ...
    def similar_records(self, text: str, k: u256) -> str:
        """Semantic neighbours from VecDB. Context only -- never an edge."""
        neighbors = self._neighbor_context(text, int(k) or MAX_KNN)
        return _canon_json({
            "items": neighbors,
            "context_only": True,
            "note": "Semantic neighbours are retrieval context. They are not evidence, "
                    "not citations, and never create graph edges.",
            "disclaimer": DISCLAIMER,
        })
